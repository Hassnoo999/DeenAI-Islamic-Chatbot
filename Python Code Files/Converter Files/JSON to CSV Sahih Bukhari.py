...
def clean_text(text: Any, default: str = "") -> str:
    """Clean text by stripping whitespace and replacing multiple spaces with single space."""
    if text is None:
        return default
...
output_dir = "Sahih Bukhari CSV"
os.makedirs(output_dir, exist_ok=True)

for i in range(1, 293):
    input_path = f"Sahih Bukhari Ahadith/sahi_bukhari_hadith_page{i}.json"  
    ...
    try:
        ...
        rag_rows = [process_hadith(h) for h in hadiths]  # List comprehension for efficiency

        if not rag_rows:
            print(f"⚠️ No data found in page {i}")
            continue

        output_path = f"{output_dir}/sahih_bukhari_page{i}.csv"
